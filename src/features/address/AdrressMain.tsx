...
export default function AddressMain() {
  ...
  /*
   * source=header means the page was opened
   * from the delivery-location header.
   *
   * In this mode:
   * - Address CRUD works
   * - Checkout is hidden
   * - createOrder is never called
   */
  const isAddressManagementMode =
    searchParams.get("source") === "header";
   ...
   * CHECK ORDERS
   *
   * Go to Profile and directly open Orders tab.
   */
    ...
    <div className="min-h-screen w-full overflow-x-hidden bg-[#FBF6EC]">
      <main className="mx-auto w-full max-w-[1400px] px-4 py-6 sm:px-6 sm:py-8 lg:px-8 lg:py-10">
        ...
        {isLoading ? (
          ...
          <div
            className={
              isAddressManagementMode
                ? "w-full"
                : "grid w-full grid-cols-1 gap-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:gap-8"
            }
          >
            {/* ADDRESS LIST */}
