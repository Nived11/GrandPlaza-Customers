...
export default function ComboOffersSection({ data = [] }: ComboOffersSectionProps) {
  return (
    <div className="w-full mt-16 lg:mt-24">
      
      {/* 📝 Header Section (Old UI Header) */}
      ...
      {/* 🍔 Grid Section (3 Columns on Desktop - Old UI Layout) */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 lg:gap-6">
        {data.map((item) => {
          // 🌟 New Logic for Prices from API
          const availableVariant = item.variants?.find((variant) => variant.is_available);
          const offerPrice = item.offer_price || availableVariant?.offer_price;
          const actualPrice = item.actual_price || availableVariant?.actual_price;

          // 🌟 Calculate Savings for the "SAVE ₹X" badge
          const savings = (offerPrice && actualPrice && Number(actualPrice) > Number(offerPrice))
          ...
          return (
            <div key={item.id} className="bg-white rounded-xl border border-[var(--brand-gold)]/20 shadow-sm hover:shadow-lg transition-all p-3 flex flex-row gap-4 h-full items-stretch">
              ...
              <div className="relative w-[120px] lg:w-[130px] rounded-xl overflow-hidden bg-[var(--brand-cream-soft)] shrink-0">
                ...
                {/* SAVE Badge (Only if there is a discount) */}
                {savings > 0 && (
              ...
              <div className="flex flex-col flex-grow py-1">
                ...
                <div className="flex items-center gap-2 mb-2.5">
                  ...
                  {actualPrice && offerPrice && Number(actualPrice) > Number(offerPrice) && (
